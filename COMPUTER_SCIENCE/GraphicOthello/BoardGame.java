    ...
    protected Move machineGreedyMove() {
        ...
        for (Move m : moves) {
            ...
            if(m.getCol()==0  && m.getRow()==0 || m.getCol()== 7 && m.getRow()==7 ||m.getCol()==0  && m.getRow()==7 || m.getCol()==7  && m.getRow()==0 )
            {
             ...
             if(m.getCol()==0  && m.getRow()==2 || m.getCol()== 0 && m.getRow()==5 ||m.getCol()==2  && m.getRow()==0 || m.getCol()==2  && m.getRow()==7 ||m.getCol()==5  && m.getRow()==0 || m.getCol()== 5 && m.getRow()==7 ||m.getCol()==7  && m.getRow()==2 || m.getCol()==7  && m.getRow()==5 )
            {
                 currentScore = 66;
                if (score >= currentScore) {
                candidateMove = m;
            ...
            if(m.getCol()==2  && m.getRow()==2 || m.getCol()== 5 && m.getRow()==5 ||m.getCol()==2  && m.getRow()==5 || m.getCol()==5  && m.getRow()==2 )
            {
                currentScore = 65;
                if (score >= currentScore) {
                candidateMove = m;
