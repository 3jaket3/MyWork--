%{
    ...
    11/6/2016   Jake Tully

    This script plots the design of two filters
    and then the combination of the filters
...
a1 = (C1*s + (1/R2) +(1/R1))./ ( C1*s +(1/R2)); % filter 1
A1 = 20*log10(a1);
...
a2 = -(C1*R2.*s)./(( 1 + C1.*s*R1).*( 1 + C2.*s*R2)); %filter 2
